...
const LeaderBoard = () => {
  ...
  useEffect(() => {
    if (state?.carsData && state.timingDataF1) {
      if (orderedList.length == 0) setOrderedList(Object.keys(state?.timingDataF1))
      orderedList.sort((a, b) => {
        if (state.sessionInfo?.Type == "Practice" || state.sessionInfo?.Type == "Qualifying") {
          if (state.timingStats[a].PersonalBestLapTime.Value == '' || state.timingStats[b].PersonalBestLapTime.Value == '') {
            return state.stints[a].Line - state.stints[b].Line
          }
          return parseTime(state.timingStats[a].PersonalBestLapTime.Value) - parseTime(state.timingStats[b].PersonalBestLapTime.Value)
        } else {
          if (state.timingDataF1[a].GapToLeader == "" || state.timingDataF1[a].GapToLeader.includes("LAP")) return -1
          if (state.timingDataF1[b].GapToLeader == "" || state.timingDataF1[b].GapToLeader.includes("LAP")) return 1
          ...
          if (isLappedB && isLappedB) return state.stints[a].Line - state.stints[b].Line

          return gapToNumber(state.timingDataF1[a].GapToLeader) - gapToNumber(state.timingDataF1[b].GapToLeader)
  ...
  return (
    <div className={`border-4 ${statusStyle}`} >
      ...
      <ul ref={parent}>
        {state && orderedList.map((carNumber, index) => {
          return <DriverRow
